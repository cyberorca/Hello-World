...
window.GAMLibrary = {
    ...
    gamSlotRenderEnded: function(event) {
        ...
        // START - TWEAK THE 336x280 LARGE RECTANGLE MASTHEAD FOR TRSTDLY ONLY
        if (pageKlyObj.site == 'trstd.ly' && containerId.includes("masthead")) {
        ...
        // END - TWEAK THE 336x280 LARGE RECTANGLE MASTHEAD FOR TRSTDLY ONLY
    ...
    for (const prop in config) {
        if (!config[prop]['requested']) {
            if (config[prop]['sizes'] == 'OOP') {
                ...
                // ADJUST THE 336x280 LARGE RECTANGLE MASTHEAD IF NOT TRSTDLY
                configSizes = pageKlyObj.site !== 'trstd.ly' && config[prop]['placeholder'].includes("masthead") ? config[prop]['sizes'][0] : config[prop]['sizes'];
                config[prop]['slot'] = googletag.defineSlot(config[prop]['adunit'], configSizes, config[prop]['placeholder']).setTargeting('position', ['2']).addService(googletag.pubads());
            }
        }
    }
}
